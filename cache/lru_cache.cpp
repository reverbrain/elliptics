...
lru_cache_t::lru_cache_t(struct dnet_node *n, size_t max_size) :
m_need_exit(false),
m_node(n),
m_cache_size(0),
m_max_cache_size(max_size) {
    m_lifecheck = std::thread(std::bind(&lru_cache_t::life_check, this));
}
...
void lru_cache_t::stop() {
    m_need_exit = true;
}

int lru_cache_t::write(const unsigned char *id, dnet_net_state *st, dnet_cmd *cmd, dnet_io_attr *io, const char *data) {
...
std::shared_ptr<raw_data_t> lru_cache_t::read(const unsigned char *id, dnet_cmd *cmd, dnet_io_attr *io) {
    const bool cache = (io->flags & DNET_IO_FLAGS_CACHE);
    const bool cache_only = (io->flags & DNET_IO_FLAGS_CACHE_ONLY);
    (void) cmd;
...
int lru_cache_t::remove(const unsigned char *id, dnet_io_attr *io) {
    const bool cache_only = (io->flags & DNET_IO_FLAGS_CACHE_ONLY);
...
int lru_cache_t::lookup(const unsigned char *id, dnet_net_state *st, dnet_cmd *cmd) {
    int err = 0;
...
iset_t::iterator lru_cache_t::create_data(const unsigned char *id, const char *data, size_t size, bool remove_from_disk) {
...
iset_t::iterator lru_cache_t::populate_from_disk(elliptics_unique_lock<std::mutex> &guard, const unsigned char *id, bool remove_from_disk, int *err) {
    if (guard.owns_lock()) {
        guard.unlock();
    }
...
void lru_cache_t::resize(size_t reserve) {
...
void lru_cache_t::erase_element(data_t *obj) {
    elliptics_timer timer;
...
void lru_cache_t::sync_element(const dnet_id &raw, bool after_append, const std::vector<char> &data, uint64_t user_flags, const dnet_time &timestamp) {
    local_session sess(m_node);
...
void lru_cache_t::sync_element(data_t *obj) {
    struct dnet_id raw;
    memset(&raw, 0, sizeof(struct dnet_id));
...
void lru_cache_t::sync_after_append(elliptics_unique_lock<std::mutex> &guard, bool lock_guard, data_t *obj) {
    elliptics_timer timer;
...
void lru_cache_t::life_check(void) {
    while (!m_need_exit) {
