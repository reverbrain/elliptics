...
void elliptics_node::read_data_wait(struct dnet_id &id, void *data, uint64_t offset, uint64_t size)
{
...
{
	int err = dnet_write_data_wait(node, NULL, 0, &id, data, offset, size, NULL, aflags, ioflags);
	if (err < 0)
		throw err;
	return err;
}
...
int elliptics_node::write_data_wait(void *remote, unsigned int remote_size, void *data, uint64_t offset, uint64_t size,
		unsigned int aflags, unsigned int ioflags)
{
	int err = dnet_write_data_wait(node, remote, remote_size, NULL, data, offset, size, NULL, aflags, ioflags);
	if (err < 0)
		throw err;
	return err;
}
