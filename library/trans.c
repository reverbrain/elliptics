...
void dnet_trans_destroy(struct dnet_trans *t)
{
	...
	if (st && st->n && t->command != 0) {
		...
		if ((t->command == DNET_CMD_READ) || (t->command == DNET_CMD_WRITE)) {
			...
			snprintf(io_buf, sizeof(io_buf), ", io-offset: %llu, io-size: %llu, io-user-flags: 0x%llx, ts: %ld.%06ld '%s.%06lu'\n",
				(unsigned long long)local_io->offset, (unsigned long long)local_io->size, (unsigned long long)local_io->user_flags,
