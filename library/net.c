...
int dnet_socket_create(struct dnet_node *n, const struct dnet_addr *addr, struct dnet_addr_socket **sockets, int num, int listening)
{
	...
	*sockets = remote;
	...
	for (i = 0; i < num; ++i) {
		...
		if (st) {
			dnet_log(n, DNET_LOG_NOTICE, "Address %s already exists in route table", dnet_server_convert_dnet_addr(&addr[i]));
			dnet_state_put(st);
			rem->s = -EEXIST;
		} else {
			good_num++;
	...
	if (good_num == 0) {
		dnet_log(n, DNET_LOG_NOTICE, "All %d nodes are already exist in route table", num);
		err = -EEXIST;
		goto err_out_exit;
	}
