...
int dnet_locks_init(struct dnet_node *n, int num)
{
	...
	if (err) {
		err = -err;
		dnet_log(n, DNET_LOG_ERROR, 0, "Could not create lock: %s [%d]\n", strerror(-err), err);
	...
	for (i = 0; i < num; ++i, ++entry) {
		...
		if (err) {
			err = -err;
			dnet_log(n, DNET_LOG_ERROR, 0, "Could not create lock %d/%d: %s [%d]\n", i, num, strerror(-err), err);
		...
		if (err) {
			err = -err;
			dnet_log(n, DNET_LOG_ERROR, 0, "Could not create cond %d/%d: %s [%d]\n", i, num, strerror(-err), err);
...
static void dnet_oplock_remove_nolock(struct dnet_node *n, struct dnet_locks_entry *entry)
{
	...
	if (!entry->lock_tree_entry.rb_parent_color) {
		dnet_log(n, DNET_LOG_ERROR, 0, "%s: trying to remove non-existen oplock.\n",
...
static struct dnet_locks_entry *dnet_oplock_ensure(struct dnet_node *n, struct dnet_id *id)
{
	...
	if (entry) {
		...
		dnet_log(n, DNET_LOG_ERROR, id->trace_id, "%s: oplock list is empty.\n", dnet_dump_id(id));
	}
...
static struct dnet_locks_entry *dnet_oplock_take(struct dnet_node *n, struct dnet_id *id)
{
	...
	if (!entry) {
		dnet_log(n, DNET_LOG_ERROR, id->trace_id, "%s: lock not found.\n", dnet_dump_id(id));
