...
static struct dnet_raw_id *dnet_ids_init(struct dnet_node *n, const char *hdir, int *id_num, unsigned long long storage_free, int group_id, struct dnet_addr *cfg_addrs, char* remotes)
{
	int fd, err, num;
	const char *file = "ids";
...
struct dnet_node *dnet_server_node_create(struct dnet_config_data *cfg_data, struct dnet_config *cfg, struct dnet_addr *addrs, int addr_num)
{
	...
	if (cfg->flags & DNET_CFG_JOIN_NETWORK) {
		...
		ids = dnet_ids_init(n, cfg->history_env, &id_num, cfg->storage_free, cfg->group_id, cfg_data->cfg_addrs, cfg_data->cfg_remotes);
		if (!ids)
