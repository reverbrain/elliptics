  do
	...
	function elliptics_proto.dissector(buffer, pinfo, tree)
		...
		while offset + header_size <= buf_len do
			...
			local packet = buffer:range(offset, packet_size)
			
			local item = tree:add(elliptics_proto, packet(), "Elliptics CMD Header")
			
			local dnet_id = item:add(elliptics_proto, packet(0, 72), "dnet_id")
			dnet_id:add(packet(0, 64), "id: " .. packet(0, 64))
			dnet_id:add(packet(64, 4), "group_id: " .. packet(64, 4):le_uint())
			dnet_id:add(packet(68, 4), "type: " .. packet(68, 4):le_uint())
			
			local cmd_id = packet(76, 4):le_uint()
			...
			item:add(packet(72, 4), "status: " .. packet(72, 4):le_uint())
			item:add(packet(76, 4), "cmd: " .. cmd.label .. " (" .. cmd_id .. ")")
			item:add(packet(80, 8), "flags: " .. packet(80, 8))
			item:add(packet(88, 8), "trans: " .. packet(88, 8))
			item:add(packet(96, 8), "size: " .. packet(96, 4):le_uint())
			if data_size > 0 then
				item:add(packet(104, data_size), "data: " .. packet(104, data_size))
			end
