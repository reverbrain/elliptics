  do
	...
	function elliptics_proto.dissector(buffer, pinfo, tree)
		...
		while offset + header_size <= buf_len do
			...
			dnet_id:add(buffer(offset + 0, 64), "id: " .. buffer(offset + 0, 64))
			...
			item:add(buffer(offset + 80, 8), "flags: " .. buffer(offset + 80, 8))
			item:add(buffer(offset + 88, 8), "trans: " .. buffer(offset + 88, 8))
			item:add(buffer(offset + 96, 8), "size: " .. buffer(offset + 96, 4):le_uint())
			if data_size > 0 then
				item:add(buffer(offset + 104, data_size), "data: " .. buffer(offset + 104, data_size))
			end
