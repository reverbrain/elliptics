...
, m_server(*this, get_monitor_port(n), cfg->family)
{
	...
	if (cfg->handystats_config != nullptr) {
		...
		if (HANDY_CONFIG_FILE(cfg->handystats_config)) {
			dnet_logger_write(cfg->log, DNET_LOG_INFO, "monitor: initializing stats subsystem, config file '%s'", cfg->handystats_config);
		} else {
			dnet_logger_write(cfg->log, DNET_LOG_ERROR, "monitor: initializing stats subsystem, error parsing config file '%s', using defaults", cfg->handystats_config);
		}
	} else {
		dnet_logger_write(cfg->log, DNET_LOG_INFO, "monitor: initializing stats subsystem, no config file specified, using defaults");
	}
	HANDY_INIT();
#else
	dnet_logger_write(cfg->log, DNET_LOG_INFO, "monitor: stats subsystem disabled at compile time");
#endif
}
...
static void init_io_stat_provider(struct dnet_node *n, struct dnet_config *cfg) {
	try {
		...
		dnet_logger_write(cfg->log, DNET_LOG_ERROR, "monitor: failed to initialize io_stat_provider: %s.", e.what());
	}
}
...
static void init_backends_stat_provider(struct dnet_node *n, struct dnet_config *cfg) {
	try {
		...
		dnet_logger_write(cfg->log, DNET_LOG_ERROR, "monitor: failed to initialize backends_stat_provider: %s.", e.what());
	}
}
...
static void init_procfs_provider(struct dnet_node *n, struct dnet_config *cfg) {
	try {
		...
		dnet_logger_write(cfg->log, DNET_LOG_ERROR, "monitor: failed to initialize procfs_stat_provider: %s.", e.what());
	}
}
...
static void init_top_provider(struct dnet_node *n, struct dnet_config *cfg) {
	try {
		...
		if (top_loaded && monitor_cfg) {
			dnet_logger_write(cfg->log, DNET_LOG_INFO, "monitor: top provider loaded: top length: %lu, events size: %lu, period: %d",
			       monitor_cfg->top_length, monitor_cfg->events_size, monitor_cfg->period_in_seconds);
		} else {
			dnet_logger_write(cfg->log, DNET_LOG_INFO, "monitor: top provider is disabled");
		}

	} catch (const std::exception &e) {
		dnet_logger_write(cfg->log, DNET_LOG_ERROR, "monitor: failed to initialize top_stat_provider: %s.", e.what());
	}
}
...
int dnet_monitor_init(struct dnet_node *n, struct dnet_config *cfg) {
	if (!get_monitor_port(n) || !cfg->family) {
		n->monitor = NULL;
		dnet_logger_write(cfg->log, DNET_LOG_ERROR, "monitor: monitor hasn't been initialized because monitor port is zero.");
		return 0;
	}
	...
	try {
		...
		dnet_logger_write(cfg->log, DNET_LOG_ERROR, "monitor: failed to initialize monitor on port: %d: %s.", get_monitor_port(n), e.what());
		return -ENOMEM;
	}
